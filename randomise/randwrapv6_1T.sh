...
mask=$5;
perms=$6;
echo "randomise -i $data -o pos-$outtag -d $des -t $con -m $mask -n $perms -T -D -v 6"
randomise -i $data -o pos-$outtag -d $des -t $con -m $mask -n $perms -T -D -v 6

## one tail only
#echo fslmaths $data -mul -1 neg-"$outtag-$filename"
#fslmaths $data -mul -1 neg-"$outtag-$filename"
#echo randomise -i neg-$outtag-$filename -o neg-$outtag -d $des -t $con -m $mask -n $perms -T -D -v 6
#randomise -i neg-$outtag-$filename -o neg-$outtag -d $des -t $con -m $mask -n $perms -T -D -v 6
...
echo fslmaths pos-"$outtag"_tstat1.nii.gz -mul posmask-$outtag "$outtag"_tstat1_Corrected.nii.gz
fslmaths pos-"$outtag"_tstat1.nii.gz -mul posmask-$outtag "$outtag"_tstat1_Corrected.nii.gz
