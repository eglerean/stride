...
mask=$5;
perms=$6;
echo "randomise -i $data -o pos-$outtag -d $des -t $con -m $mask -n $perms -T -D"
randomise -i $data -o pos-$outtag -d $des -t $con -m $mask -n $perms -T -D
echo fslmaths $data -mul -1 neg-"$outtag-$filename"
fslmaths $data -mul -1 neg-"$outtag-$filename"
echo randomise -i neg-$outtag-$filename -o neg-$outtag -d $des -t $con -m $mask -n $perms -T -D
randomise -i neg-$outtag-$filename -o neg-$outtag -d $des -t $con -m $mask -n $perms -T -D
